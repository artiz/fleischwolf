...
l box = λ iou l iou + λ l 1 (1) l = λl + (1 − λ ) l box s
...
Image width and height ≤ 1024 pixels (2) Structural tags length ≤ 512 tokens.
...
EditDist ( Ta , Tb Tb ) TEDS ( Ta Ta , Tb Tb ) = 1 − max ( | Ta | Ta | Tb | ) (3) Ta , Tb
...
alignment = arg min { D c } c (4) D c = max { x c } − min { x c }
